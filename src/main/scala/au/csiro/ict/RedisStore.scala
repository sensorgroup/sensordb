          ...
          if (kv._2 != null){
            ...
            if (columns.isEmpty || (columns.get._1 <= key && columns.get._2 >= key)) {
              if (level == RawLevel)
                ...
                val List(min,max,count,sum,sumSq) = parse[List[Double]](Bytes.toString(kv._2))
                chunker.insert(sid,level.colIndexToTimestamp(current,currentInTs,key), min,max,count,sum,sumSq)
              }
            }
          }
        }
