...
import au.csiro.ict.{Utils, Cache, StorageIdGenerator, SensorDB}
...
class SampleSetup extends ScalatraSuite with FunSuite {
  ...
  session{
    ...
    post("/experiments",Map("name"->"Ginninderra","timezone"->"1000")){
      body should include ("_id")
      experimentId=idOf(body)
      status should equal(200)
    }
